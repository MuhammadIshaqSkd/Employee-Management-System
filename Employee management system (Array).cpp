...
struct Employee
	{
		...
		{
			fstream write_data;
		    write_data.open("d:\\Recorded.txt",ios::app|ios::binary);
			set_rec();
		...
		void read_object_data()		// Read Data from File
		{
			system("cls");
			fstream read_data;		
			read_data.open("d:\\Recorded.txt",ios::app|ios::in|ios::out|ios::binary);
		...
		void search_records()
		{
		   system("cls");
		   fstream search_data;
		   search_data.open("d:\\Recorded.txt",ios::in |ios::binary);
		   int x;
		...
		void update_data()
		{
			...
			cin>>sd;
			f.open("d:\\Recorded.txt ",ios::in|ios::binary |ios::in);
	...
	int main()
	{
		...
		{ 
		  ...
		  cout<<"\t\t\t3 for Update Employee Record \n\t\t\t4 for Search Existing Record"<<endl;
	     cin>>x;
	     switch(x)
		    {
		     case 1:
			  {
					...
					char ch;
				 do{                          	
		             e[i].stor_object_data(); 	 // Entering Data in Record
					 i++;
					 cout<<"Press 1 for Enter More Record and 0 For Exit "<<endl;
					 cin>>n;
					 if(n!=1)
					 cout<<"\n\t\tData id store Successfully"<<endl;
					 break;       	
				   }while(i<50);
				}break;
				case 2:
				{
					e[50].read_object_data();
				}break;
				case 3:
				{
				  do
